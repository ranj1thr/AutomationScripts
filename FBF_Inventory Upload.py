...
# Target Table Name
TABLE_NAME = "FBF_Inventory"

# Folder path where CSV files are stored
BASE_FOLDER = r"C:\Users\Ranjith\OneDrive - MSFT\Shakedeal Documents\Report Files\FC_Inventory\Flipkart"
...
def create_fbf_inventory_table(df):
    """Dynamically create the 'FBF_Inventory' table based on the CSV file structure."""
    columns_with_types = []
...
def load_data_into_db(file_path):
    """Load data from CSV into the `FBF_Inventory` table while handling encoding issues."""
...
def clear_table():
    ...
    with engine.begin() as conn:
        try:
            ...
            print(f"✅ Table `{TABLE_NAME}` truncated (all data removed).")
        except Exception as e:
...
def main():
    ...
    clear_table()  # Truncate table before inserting new data
